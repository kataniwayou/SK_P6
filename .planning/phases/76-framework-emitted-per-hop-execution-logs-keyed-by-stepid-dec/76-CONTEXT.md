...
- **D-11:** **One record per fan-out EDGE (per next step)**, not one per pre-pipeline run — forced by ANL-03: the record's purpose is the causal edge `inbound EntryId → the specific next step`. A per-run record listing all next steps would flatten the edges and lose the one→one mapping. Lands inside the orchestrator Pre's existing next-step loop (`OrchestratorPrePipeline.cs:126-145`, `foreach (var (stepId, step) in selection.Matches)`). A 2-way fan-out at C emits exactly 2, distinguished by next `StepId` (FW-02 acceptance).
  - **⚠ RESOLVED 2026-07-16 (research Discrepancy 1 — Option C):** D-11's original field list assumed the Pre loop "mints a messageId per next step." It does NOT — `OrchestratorPrePipeline.cs` sends `NextStepHandoff` with **no MessageId override** (Phase-72 D-11 mechanism); MassTransit assigns the outbound envelope id opaquely, concretized only downstream at `RelocateTail`. The outbound `MessageId` (M_{N+1}) is **not in hand** at the fan-out loop. **Resolution: the fan-out edge record carries FIVE fields — `(CorrelationId, ExecutionId, WorkflowId, inbound EntryId, next StepId)` — the outbound MessageId is OMITTED.** This fully satisfies ANL-02 (expected set = next stepIds) and ANL-03 (inbound `EntryId` = M_N proves hop N ran); the dropped field was only a forward-linking convenience. Phase-72 D-11's "no override" stays intact (no reversal).
...
- **D-14:** Emission site: `OrchestratorPrePipeline`, where next-step resolution already happens (same identities-in-hand argument as D-08). Fan-out edge record → the `:126-145` next-step loop; terminal-reached record → the `:87-93` completed-terminal branch (`selection.Matches.Count == 0 && selection.UnresolvedIds.Count == 0`; do NOT emit on the `:73-80` L1-miss or `:113-119` clean-absent branches). Per D-11's Option-C resolution the emission is NOT at `RelocateTail` (that site lacks the inbound M_N).

### Outcome fidelity (SPEC FW-01)
- **D-18 (RESOLVED 2026-07-16 — research Design Point B):** The true terminal outcome for the normal path is decided **inside** `OutputTail.RunAsync` — it downgrades `Completed → Failed` when the output blob fails schema validation (`OutputTail.cs:57-59`). A per-hop record emitted at the `ProcessorPipeline` site (D-08) would otherwise log the optimistic pre-tail `Completed` even when the wire sends `StepFailed`. **Resolution: change `OutputTail.RunAsync` to return the resolved outcome** (currently returns `bool proceed` → return `(bool proceed, StepOutcome resolved)` or an `out StepOutcome`), so the pipeline logs the TRUE terminal outcome. Applies only to the normal-completion branch (`ProcessorPipeline.cs:162-173`); the other terminal branches already hold their resolved outcome directly.

...
**Success Criteria** (what must be TRUE):
  ...
  3. The runtime dashboard renders live GC / memory / thread-pool / exception / uptime panels for all four service classes, broken down per selected pod (RTD-01, RTD-02, RTD-03).
  4. The business dashboard renders the nine pipeline counters with consumed-vs-sent conservation legible and the fault indicators (`orchestrator_step_unresolved_total`, `processor_spawn_dropped_total`, keeper drop outcomes) on dedicated panels; the WebApi row shows ASP.NET Core request metrics (BPD-01, BPD-02, BPD-03).
  ...
  7. With the k8s stack up and pipeline traffic flowing, every panel on both dashboards renders non-empty data for each selected service (VER-01).

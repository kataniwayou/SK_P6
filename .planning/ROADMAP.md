...
- 🚧 **v11.0.0 Framework Data-Channel & Processor Boundary Hardening** — Phases 84-85 (in progress — started 2026-07-21) — foundational widening of the framework data channel from `string` to `byte[]` (Phase 84 ✅ shipped, verified SOLELY by the 7-scenario sweep reproducing all-PASS), then processor↔framework boundary hardening (Phase 85): fail-loud `SpawnToPost` + nack-on-spawn-exhaust (ack only when all spawns succeed) + framework-owned entry deletion with the concrete `DeleteEntry` surface removed; DATA-01…05, PB-01…03. **Kafka import/export (KIMP-01…06, MANIP-01, KEXP-01) deferred to a future milestone** — the byte[] channel is its prerequisite, now shipped.

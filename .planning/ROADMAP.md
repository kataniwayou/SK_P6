...
- ✅ **v12.0.0 Resilience & Health-Probe Hardening** — Phase 86 (SHIPPED 2026-07-27, tag `v12.0.0`; archive: [milestones/v12.0.0-ROADMAP.md](milestones/v12.0.0-ROADMAP.md)) — refactor console + webapi health probes so infrastructure failures (Postgres/Redis/RabbitMQ) never crash or restart pods (only a genuine watchdog-loop stall restarts), via one shared `BaseConsole` liveness-watchdog primitive + a hard readiness latch (no self-heal) + Redis-on-readiness + processor identity/schema-on-readiness + new k8s startupProbes. Design spec: [docs/superpowers/specs/2026-07-26-console-health-liveness-refactor-design.md](../docs/superpowers/specs/2026-07-26-console-health-liveness-refactor-design.md)
- 🚧 **v13.0.0 Observability Dashboards** — Phase 87 (started 2026-07-27) — two Grafana dashboards over the telemetry the stack already emits: **runtime instrumentation** across all four service classes, and **business/pipeline** counters across the three pipeline services, each with a multi-select `source` dropdown and a multi-select pod (`service_instance_id`) dropdown. Grafana deployed into the existing k8s stack; dashboards ship as **checked-in portable JSON only** (file-provisioned, no PVC, no UI as source of truth). No production source changes — every label needed is already promoted by the collector's `resource_to_telemetry_conversion`. DASH-01…04, RTD-01…03, BPD-01…03, VAR-01…04, VER-01…02.

## 🚧 v13.0.0 Observability Dashboards (IN PROGRESS — started 2026-07-27)

**Milestone Goal:** Make the telemetry the stack already emits *readable*. Everything through v12.0.0 has produced signal — runtime instrumentation on all four services, domain counters on the three pipeline services, `source` + `service_instance_id` + `service_name` labels on every series — but the only consumers to date are the analyzer's Prometheus queries and ad-hoc PromQL. This milestone puts a Grafana in the k8s stack and lands two dashboards over that signal: one **runtime** (all four service classes), one **business/pipeline** (the three services that emit domain counters). Both are filtered by the same pair of multi-select dropdowns — service class (`source`) and pod (`service_instance_id`) — so a single misbehaving replica is separable from its peers.

**Portability is a first-class constraint:** dashboards are checked-in JSON, file-provisioned from a ConfigMap, with the datasource behind a template variable and **no Grafana PVC**. A pod delete/recreate must reproduce them exactly from the repo; the Grafana database is never the source of truth.

**No production source changes.** The collector already runs `resource_to_telemetry_conversion: true` (`k8s/02-configmaps.yaml:85`), which is what makes `source` and `service_instance_id` Prometheus labels. If a dashboard wants a label or series that does not exist, that is a finding to report — not an instrumentation change to slip into a dashboards milestone.

**Known asymmetry (accepted, not a defect to fix here):** the WebApi emits **no** domain counters and not even the MassTransit meter — its metrics registration (`ObservabilityServiceCollectionExtensions.cs:96-118`) is AspNetCore + HttpClient + Runtime only. So it appears in full on the runtime dashboard and only via ASP.NET Core request metrics on the business dashboard. Adding WebApi business counters was explicitly considered and placed **out of scope** (see REQUIREMENTS.md).

**Numbering:** continued from v12.0.0 (ended at Phase 86) → **87**.
...
**Goal**: Grafana runs in the `skp` namespace as part of `kubectl apply -k k8s/`, with the in-cluster Prometheus provisioned as its default datasource and dashboards file-provisioned from checked-in JSON (no PVC — the repo is the single source of truth, and a pod recreate reproduces them exactly). Two dashboards ship: (1) **Runtime instrumentation** covering all four service classes (`webapi`, `orchestrator`, `keeper`, `processor`) from the shared `OpenTelemetry.Instrumentation.Runtime` series (Prometheus family `process_runtime_dotnet_*`, **not** `dotnet_*`) — GC by generation, heap size/fragmentation, GC-committed memory, thread pool, exception rate, JIT activity, plus honestly-titled proxies where working set / process CPU / uptime do not exist in this stack; (2) **Business/pipeline** covering the domain counters of the three pipeline services (`orchestrator_messages_consumed/sent_total` + `orchestrator_step_unresolved_total`, `keeper_messages_consumed/sent_total` + `keeper_l2_probe_total`, `processor_messages_consumed/sent_total` + `processor_spawn_dropped_total`), with consumed-vs-sent conservation legible at a glance and the fault indicators on their own panels; the WebApi appears there via ASP.NET Core request metrics only. Both dashboards carry the same two `label_values()`-populated multi-select template variables — `source` (service class) and `service_instance_id` (pod) — with the pod variable chained to the source selection, plus a datasource variable so the JSON loads unmodified into any Grafana. No production source code is touched.
**Depends on**: — (first phase of v13.0.0; builds on the shipped v10.0.0 k8s deployment and the metrics pipeline proven live in quick task 260727-ngv)
...
> **Amended 2026-07-27** — the goal text and success criteria above were re-synced with the REQUIREMENTS.md amendment of the same date (commit `3f10ded`), after Phase-87 research live-verified the metric inventory and five requirement premises did not survive: the runtime family name, the non-existence of CPU/uptime/working-set, the non-existence of any keeper reinject/drop counter, `service_name`'s uselessness as a processor discriminator, and the unsatisfiability of "every panel non-empty" for fault counters on a healthy stack. Evidence: [`phases/87-grafana-observability-dashboards/87-RESEARCH.md`](phases/87-grafana-observability-dashboards/87-RESEARCH.md) (F-1…F-8, OQ-1…OQ-5).
**Success Criteria** (what must be TRUE):

  1. `kubectl apply -k k8s/` brings up Grafana in `skp` with Prometheus already wired as the default datasource and both dashboards already present — zero manual UI setup (DASH-01, DASH-02, DASH-03).
  2. Deleting the Grafana pod and letting it recreate reproduces both dashboards identically from the repo JSON, proving there is no PVC and no UI-side source of truth (DASH-03, VER-02).
  3. The runtime dashboard renders live GC / memory / thread-pool / exception / JIT panels for all four service classes, broken down per selected pod, with the working-set / CPU / uptime axes shipped as honestly-titled proxies (no panel titled "CPU", "Uptime", or "Working Set") (RTD-01, RTD-02, RTD-03).
  4. The business dashboard renders the nine pipeline counters with consumed-vs-sent conservation legible and the fault indicators (`orchestrator_step_unresolved_total`, `processor_spawn_dropped_total`, and the keeper consumed−sent gap plus `keeper_l2_probe_total` heartbeat — no reinject/drop counter exists) on dedicated panels; the WebApi row shows ASP.NET Core request metrics (BPD-01, BPD-02, BPD-03).
  5. On both dashboards, the `source` dropdown lists the four service classes and the pod dropdown lists only the pods of the currently-selected source(s); both are multi-select with "All", both are `label_values()`-driven, and every panel honors both selections (VAR-01, VAR-02, VAR-03).
  6. Each dashboard JSON references its datasource through a template variable (no hardcoded UID), so the file is portable to another Grafana without editing (DASH-04).
  7. With the k8s stack up and pipeline traffic flowing, every panel on both dashboards renders data for each selected service under VER-01's two-class rule — Class A (runtime + steady-state pipeline counters) returns ≥1 real sample; Class B (fault indicators, guarded by `or vector(0)`) may legitimately render `0`. The pod dropdown is asserted as a superset of the live pod set, not an exact match (VER-01).
...
**Plans**: 11 plans in 10 waves (all live scenarios are strictly sequential — they mutate the same `skp` cluster)
  ...
  - [x] 88-02-PLAN.md — Wave 1: the runtime-only fault mechanism (`scripts/lib/phase-88-cluster-ops.ps1`) — live-read replica capture, scale sequencer, `kubectl set env` seam arm/disarm, `Assert-StackRestored`; rejects the stale `$TierReplicas` map (DISC-05, DISC-06)
  ...
  - [x] 88-04-PLAN.md — Wave 3: driver frame + static scenario table + `-Mode Baseline`; captures the DISC-01 healthy band for all fourteen panels over ten equal 60 s pinned sub-windows at a fixed viewport (DISC-01, DISC-03)
  - [x] 88-05-PLAN.md — Wave 4: `-Mode Scenario` engine (arm → settle ≥150 s → re-baseline → trigger → after → restore, cross-talk, rollout discontinuity) + WEB-01 (panels 10, 11, 12, 14) and WEB-02 (panel 13) (DISC-02, DISC-03, DISC-06)
  - [x] 88-06-PLAN.md — Wave 5: scale-lever scenarios SCALE-01/02/03 — panels 3, 4, 5 under a processor outage, panel 1 under an orchestrator outage, panel 9's "No data" under a keeper outage; orchestrator HA replica count preserved (DISC-02, DISC-03, DISC-05, DISC-06)
  ...
  - [x] 88-11-PLAN.md — Wave 10: blocking human checkpoint — attributed sign-off on the runbook and the readiness verdict, or findings recorded verbatim for gap closure (HAND-01, HAND-02)

## ✅ v12.0.0 Resilience & Health-Probe Hardening (SHIPPED — 2026-07-27, tag `v12.0.0`)

_Archived: [milestones/v12.0.0-ROADMAP.md](milestones/v12.0.0-ROADMAP.md) · [milestones/v12.0.0-REQUIREMENTS.md](milestones/v12.0.0-REQUIREMENTS.md). Full phase detail below is retained as historical record._

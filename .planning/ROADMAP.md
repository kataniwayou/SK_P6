...
- 🚧 **v11.0.0 Kafka Binary Import/Export** — Phases 84-85 (in progress — started 2026-07-21) — foundational widening of the framework data channel from `string` to `byte[]` (Phase 84, framework-only, verified SOLELY by the existing 7-scenario fault-recovery sweep reproducing all-PASS), then a Mode-2 `KafkaImporter` entry-step processor that pull-consumes binary files from a standalone Kafka container, writes bytes directly to L2, and commits each Kafka offset per-message only after a confirmed store + hand-off (Phase 85); DATA-01…05, KIMP-01…06
...
#### Phase 85: KafkaImporter Processor
**Goal**: A `KafkaImporter` processor imports binary files from Kafka into the pipeline over the Phase-84 `byte[]` channel. It is a **Mode-2 entry step** triggered by the scheduler with `executionId == Guid.Empty`; on each dispatch it pull-consumes up to N messages (each a binary file) from a configured topic + consumer group, mints a fresh `executionId` per file, writes the file's bytes **directly to L2** (never on the message bus — the dispatch/hand-off messages stay small JSON: config + coordination ids only), hands off a reference downstream, and **commits that Kafka message's offset ONLY after its L2 store + hand-off are confirmed successful** (per-message commit, fail-loud — replacing `SpawnToPost`'s best-effort swallow; any failure leaves the offset uncommitted so the message and the remainder of the batch redeliver on the next dispatch). The importer is content-agnostic (moves bytes topic→L2 without inspecting file type/content). Kafka runs as a **standalone container** (not a k8s workload), reached from within the cluster via a configured bootstrap address `host.docker.internal:9092` with the broker's advertised listeners set so the in-cluster client can reconnect. Its `ProcessorConfig` carries the topic name, consumer group, and batch size N (deriving from the framework `ProcessorConfig` marker, like `SampleConfig`).
**Depends on**: Phase 84 (the importer writes binary file bytes into the widened `byte[]` L2 channel — that channel must exist first)
**Requirements**: KIMP-01, KIMP-02, KIMP-03, KIMP-04, KIMP-05, KIMP-06
**Success Criteria** (what must be TRUE):
  1. The KafkaImporter is triggered by the scheduler as a Mode-2 entry step (`executionId == Guid.Empty`) and pull-consumes up to N messages per dispatch from a configured topic + consumer group. (KIMP-01, KIMP-06)
  2. Each consumed binary file becomes one execution with a freshly-minted `executionId`; its bytes are written directly to L2 and never carried on the message bus (the dispatch/hand-off messages stay small JSON — config + coordination ids only). (KIMP-02)
  3. The importer commits each Kafka message's offset ONLY after that file's L2 store and downstream hand-off are confirmed successful (per-message commit, fail-loud — no best-effort swallow); any failure leaves the offset uncommitted so the message and the batch remainder redeliver on the next dispatch. (KIMP-03)
  4. The importer is content-agnostic — it moves bytes from the topic to L2 without inspecting or depending on the file type or content. (KIMP-04)
  5. Kafka runs as a standalone container (not a k8s workload) reached from within the cluster via the configured bootstrap `host.docker.internal:9092` (advertised listeners set for in-cluster reconnect); the importer's `ProcessorConfig` carries topic, consumer group, and batch size N (deriving from the framework `ProcessorConfig` marker, like `SampleConfig`). (KIMP-05, KIMP-06)
**Plans**: TBD
